...
<!--  HTMLHeader.page   -->
  ...
  <link rel="shortcut icon" href="/resources/module/assets/images/mobile_icone/7272.png" />
    ...
    <link rel="stylesheet" media="all" href="${pageContext.request.contextPath}/resources/css/shake.css">
    <script src="${pageContext.request.contextPath}/resources/js/jquery-1.11.3.min.js"></script>
    <script src="${pageContext.request.contextPath}/resources/js/jquery.placeholder.enhanced-1.5.js"></script>
  ...
  <script>
    ...
    $(document).ready(function () {
      ...
      function getPrj_list(Pstate){
        $.ajax({
            //*e*url:"/data.do?callcmd=PRJ_LIST",
            //url:"/common/getproject_list.do",
            url:"/common/projectList",
            type:'post',
            data:{"Prj_State":Pstate},
            success:function(data){
            project_create(data);
            ChoiceEvent();
            }
      ...
      if(Side_Menu==null)
      {
        //alert('Side_Menu...');
        $.ajax({
            //url:"/common/Sidemenu.do",
            url:"/common/sidemenu", //  leftMenu.do",
            type:'post',
            dataType : 'html',
            success:function(data){
              //console.log(data);
            $("#side_ul").html(data);
            try {
              $('.sidebar-content').localize();
              //console.log(" sidebar-content  >> after")
            }
            catch (e){}

            $.jStorage.set("SideMenu_"+gcode ,data);
            menuEvent();
            }
        });
      }else{
      ...
      if (localStorage.getItem('logo_'+gg_code)===null)
      {
        ...
        $.ajax({
          //url : "/Common/imgExist.do"
          url : "/common/fileExist"  //이녀석만 사용함..
          ,type : "post"
          ,data : { "file_path":logo_path}
          ,success : function(fileExist){
            if (fileExist=="Y")
            {
              getDataUri('/resources'+logo_path, function(dataUri) {
                  localStorage.setItem("logo_"+gg_code, dataUri);
                  $("#logo").attr("src", dataUri);
              });
            }else{
              $("#logo").attr("src", "");
            }
      ...
      if (  inside=="true" &&  '${user.projectSeqno}' != 16 )
      {
        $(".Contact-list").on("click",".user_val",function(e){
          ...
          if (confirm("선택한 사용자로 변경하시겠습니까??") ){    //확인
            ...
            $.ajax({
              url: '/user/loginChange',
              type: 'post',
              data: {  "cuid": $(this).data("userseqno")
                    , "gssid" : "${user.sessionId}"
                   },
              dataType: 'json',
              async : false,
              success: function(data) {
                switch(data)
                {
                  case 100 :  /*alert("변경완료");*/  location.reload();   break;
                  case 101 :  alert("퇴사한 사용자는 로그인할 수 없습니다.[101]"); break;
                  case 102 :  alert("참여 종료된 프로젝트에는 로그인할 수 없습니다.[102]"); break;
                  case 200 :  alert("계정 정보가 없습니다..[200]"); break;
                  case 201 :  alert("서비스가 중지된 프로젝트입니다.[201]"); break;
                  ...
                  case 300 :  alert("비정상적인 요청입니다. [300]"); break;  // 잘못된 접속입니다
                                    case 999 :
                            swal({
                                title: "해당 사용자는 메뉴 접근 권한이 없습니다. \n 메인 페이지로 이동합니다.",
                                type: "info",
                                        confirmButtonColor: "#2196F3",  // "#EF5350"
                                confirmButtonText: i18next.t("btn.ok"),
                                closeOnConfirm: true
                            },function(isConfirm){
                                if(isConfirm){
                                                location.href = "<c:url value='${user.mainPageUrl}' />";
                                }
                            });
                    break;  // 권한없는 사용자인 경우 메인 페이지로 이동
                  default :   alert("회원정보를 찾을수 없습니다."); break;
                }
              }
                /*
                     , error:function(request,status,error){
    ...
    function fn_inoutLoad(){
      //출퇴근버튼 로드
      $.ajax({
         //url : "/module/UserModule/Attendance/MAIN_Index.page"
               url : "/module/usermodule/attendance/mainIndex"
        ,type : "post"
        ,data : { }
        ,success : function(data){
          $("#inoutInfo").html(data);
  ...
  <c:if test="${user.projectSeqno eq 17 || user.projectSeqno eq 14 || user.projectSeqno eq 13  || user.projectSeqno eq 20  }">
    ...
    <script>
        ...
        function Device_info() { //단말기정보 업데이트
          ...
          OneSignal.getUserId(function(userId) {
            if (userId != null) {
              ...
              $.ajax({
                //url: '/Common/Device_info.do',
                url: '/push/deviceinfo',
                type: 'post',
                data: { "device_id": userId },
                success: function(data) {
                  // ?? 성공 여부와 상관없이 등록하네??
                  sessionStorage.setItem("player_id", userId);
                  //사용자 Tag에 사용자 sys_id 저장 // 필요에 따라 만들수 있으나, 나중에 필터하여 사용

                  OneSignal.sendTag("user_ip", "${remoteAddr}");
      ...
      function fn_NotificationOpened(noti_id) {
        ...
        */
        $.ajax({
          //url: '/Common/NotificationGetAction.do',
          url: '/push/notificationGetAction',
          type: 'post',
          ...
          success: function(data) {
            ...
            if (data.actionUrl != "") {
              eval(data.actionUrl); //처리함수 실행
            } else if (data.fromPushChecked == "") {

              //location.reload(); //메뉴이동 변수 쿠키에 세팅후 페이지 새로고침(service.do 다시 호출)
                           // alert('fn_NotificationOpened  main_seqno: '+data.menuSeqNo);
                $("#main_seqno").val(data.menuSeqNo);
                $('#actForm').attr("action","/common/menu");
                $("#actForm").submit();
