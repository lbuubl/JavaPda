...
function validation(){
	validator = $("#item_form").validate({
        ...
        messages: {
            c_nm: {
                required: i18next.t("obs:msg.comName")
            },
			c_owner: {
                required: i18next.t("obs:msg.comOwner")
            },
			c_tel: {
                required: i18next.t("obs:msg.comTel")
            }
...
function DBProcess(){
			...
			$.ajax({
				//url:"/module/sysmodule/obs/obs_c.Do",
				url:"/module/sysmodule/obs/dml/c",  // c(company)
				type:'post',
				data:$('#item_form').serialize(),
				success:function(data){
					if (data.indexOf("/") > 0)
					{
							...
							sel = sel[0];
							ps= $("#ps").val();
							ref.create_node(sel, {"id":tree_id, "text": tree_rename , "icon":" icon-home8", "parent": ps, "a_attr":{"T":"C"}, "state":{"opened":true}});

							alert(i18next.t("msg.saveOk"));
							$("#tree_md").val("");
					}else if(data=="2"){

							$('#ajax').jstree().select_node($("#cn").val());	//트리 선택
							Sel=$('#ajax').jstree().get_selected();	//트리 선택
							$('#ajax').jstree().rename_node(Sel, $("#c_nm").val());	//이름변경
