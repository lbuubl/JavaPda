define(function() {
	...
	*/
	...
	PdsViewer.prototype.save = function(formData) {
		...
		var dfd = new $.Deferred();
		$.ajax({
			url: '/community/companyPds/' + _this.seqno,
			type: 'put',
			data: formData,
			success: function(data) {
				dfd.resolve(data);
	...
	PdsViewer.prototype.open = function(seqno, grpKey) {
		...
		loader.get('community/companyPds.comment.js').then(function(comment) {
		...
		if (typeof _this.el.data('kendoWindow') === "undefined") {
			...
			loader.get('community/companyPds.revision.js').then(function(revision) {
