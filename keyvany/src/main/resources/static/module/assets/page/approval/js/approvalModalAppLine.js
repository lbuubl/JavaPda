...
//라인그룹 체인지 이벤트
	...
	$("#appLineModal").on("click",".btn-lineSave",function(e){
		$("#lineType").val($(this).data("type"));
			//console.log('$("#appLineBox option:selected")[0].value='+$("#appLineBox option:selected")[0].value);
		lineSaveMoadlLoad()
	});
	...
	$("#appLineModal").on("click",".btn-lineDel",function(e){
		...
		if ($($("#appLineBox option:selected")[0]).data("isdel")=="0")
		{
			//console.log("결재라인="+$($("#appLineBox option:selected")[0]).data("isdel"));
			showSwal(i18next.t("approval:msg.lineNotDel"),"warning");
			return false;
		}
	...
	$("#appLineSaveModal").on("click",".lineSaveSubmit",function(e){
		...
		if ($("#appLineBox option:selected")[0].value=="")
		{
			//console.log('$("#appLineBox option:selected")[0].value='+$("#appLineBox option:selected")[0].value);
		}
		...
		if($("#lineType").val()=="1"){
			objId = "#selectedApprovalBox option"
		}else{
			objId = "#receiveBox option"
		...
		$(objId).each(function(i,item){
			dataArr.push(item.title + "@@" + $(item).val() + "@@" + $(item).text());
		});
	...
	$("#appLineModal").on("click","#btn_appLineD",function(e){
		if($("#selectedApprovalBox OPTION:selected").length==0){
	...
	$("#appLineModal").on("click","#btn_authM",function(e){
		...
		for (i=0; i<objDataArray.length ;i++ )
		{
			...
			//같은 value 있는지 체크...
			if($("#viewAuthBox OPTION[value*='"+objDataArray[i][5]+"'][value*='"+objDataArray[i][8]+"']").length == 0 ){
				...
				optHtml = optHtml+"<option style='background-color:#ddd;' title='"+ objDataArray[i][9] +"' value='"+objDataArray[i][5]+"_"+objDataArray[i][6]+"_"+objDataArray[i][3]+"_M'>"+objDataArray[i][2]+"</option> " ;
			}
	...
	$("#appLineModal").on("click","#btn_authGrp",function(e){
		 ...
		 */
			...
			//같은 value 있는지 체크...
			if($("#viewAuthBox OPTION[value*='"+nSeq+"'][value*='"+t+"']").length == 0){

				//optHtml = "<option style='background-color:#ddd;' title='"+fnm+"' value='"+nSeq+"_"+nm+"_"+ocd+"_"+t+"_0'>"+nm+"</option> " ;
				optHtml = "<option style='background-color:#ddd;' title='"+fnm+"' value='"+nSeq+"_"+nm+"_"+ocd+"_G'>"+nm+"</option> " ;
				$("#viewAuthBox").append(optHtml);
			}
	...
	$("#appLineModal").on("click","#btn_authD",function(e){
		if($("#viewAuthBox OPTION:selected").length==0){
			showSwal(i18next.t("approval:msg.noSelectData"),"warning");
			return false;
		}
		$("#viewAuthBox OPTION:selected").each(function(i,item){
			$(item).remove();
		})
	})
	...
	$("#appLineModal").on("click","#btn_addAppLine",function(e){
		...
		$.ajax({
			...
			url:"/module/usermodule/approval/common/modalAddAppLine",
			type:"POST",
			dataType:"html",
	...
	$("#appLineModal").on("click","#btn_recvM",function(e){
		...
		if(nSeq!=""){
			...
			if(appRG[3] == nSeq){
				//그룹이 있다면 삭제..
				if($("#receiveBox OPTION[value*='_G']").length > 0){
					//삭제하고 새로 넣을것인지 묻기
					if(confirm(i18next.t("approval:msg.grpInfoReset"))){
						$("#receiveBox OPTION[value*='_G']").remove();
						$(objDataArray).each(function(i,item){
							...
							//optHtml = optHtml + "<option value='"+$(item)[1]+'_'+$(item)[6]+" "+$(item)[7] +'_'+$("#osq").val()+'_'+$(item)[8]+"' title='"+nm+">"+$(item)[6]+" "+$(item)[7]+"'>"+$(item)[6]+" "+$(item)[7] +"</option>";
								optHtml = optHtml + "<option value='"+$(item)[5]+'_'+$(item)[6]+" "+$(item)[7] +'_'+osq+'_'+$(item)[8]+"' title='"+nm+">"+$(item)[6]+" "+$(item)[7]+"'>"+$(item)[6]+" "+$(item)[7] +"</option>";
							}
						})
						appRG[2]="1";
					...
					//중복체크
						if($("#receiveBox OPTION[value*='"+$(item)[5]+"']").length == 0 ){
							//optHtml = optHtml + "<option value='"+$(item)[1]+'_'+$(item)[6] +'_'+$("#osq").val()+'_'+$(item)[8]+"' title='"+nm+">"+$(item)[6]+" "+$(item)[7]+"' >"+$(item)[6]+" "+$(item)[7] +"</option>";
							optHtml = optHtml + "<option value='"+$(item)[5]+'_'+$(item)[6] +'_'+osq+'_'+$(item)[8]+"' title='"+nm+">"+$(item)[6]+" "+$(item)[7]+"' >"+$(item)[6]+" "+$(item)[7] +"</option>";
						}
					})
					appRG[2]="1";
					$("#receiveBox").append(optHtml);
	...
	$("#appLineModal").on("click","#btn_recvGrp",function(e){
		...
		if(nSeq!=""){
			...
			if(confirm(i18next.t("approval:msg.settingReset"))){
				...
				//리셋
				$("#receiveBox option").remove();
				optHtml = optHtml + "<option value='"+nSeq+'_'+nm+'_'+ocd+'_'+ t +"' title='"+nm+"' >"+nm+"</option>";
				$("#receiveBox").append(optHtml);
	...
	$("#appLineModal").on("click","#btn_recvD",function(e){
		if($("#receiveBox OPTION:selected").length==0){
			showSwal(i18next.t("approval:msg.noSelectData"),"warning");
			return false;
		}
		$("#receiveBox OPTION:selected").each(function(i,item){
			$(item).remove();
		})
	})
	...
	$("#appLineModal").on("click","#setAppLineSave",function(e){
		...
		var option = $("#selectedApprovalBox option");
		...
		$("#selectedApprovalBox option").each(function(i,item){
			//결재자가 있으면서 ( 신규결재자에 사용자가 등록되있는 경우만 )
			if($(item).val().split("_")[4] == "1" && ( $(item).val().split("_")[1] !="0" || ($(item).val().split("_")[1] =="0" && $(item).val().split("_")[3] !="0" ) ) ){
				addLineCheck = true;
				return false;
			}
		});
		...
		if(lastAppLineIdx >= 0 ){
			for(var i = lastAppLineIdx ; i < option.length-1 ; i++){
				...
				if(typeof($(option[i]).val()) != "undefined"){
					item = $(option[i]).val().split("_");
...
function fn_setAppLineSave(){
	...
	$("#selectedApprovalBox option").each(function(i,item){
		...
		//console.log("idx"+idx+" = "+itemArr.join("_"));
	...
	*/
	...
	$("#viewAuthBox option").each(function(i,item){
...
function loadUserDataTable(){
	console.log ('approvalModalAppLine.js  loadUserDataTable.............');
...
function defaultAppLineInit(){
	...
	*/
	...
	for (i=0; i<hiddenApprovalBoxValue.length ;i++ ){
		...
		appLineHtml = appLineHtml + "<option title='"+hiddenApprovalBoxTitle[i]+"' data-oldvalue='"+hiddenApprovalBoxValue[i]+"' value='"+hiddenApprovalBoxValue[i]+"' " ;

		if(valArr[1] == "0"){
			appLineHtml = appLineHtml + " class='select-option-bg' "
		}
		appLineHtml = appLineHtml + ">"+hiddenApprovalBoxText[i]+"</option>";
	}
	if ($("#hiddenApprovalValue").val() != "")
	{
...
function fn_btnAppLineAdd(type){
	...
	var optLen = $("#selectedApprovalBox option").length; //option길이

	//console.log('optLen : '+optLen);
	//console.log('hiddenAppType : '+$("#hiddenAppType").val());


	if (optLen==0 && $("#hiddenAppType").val().toString()=="1")
	{
		//console.log("자유결재선");
		$.ajax({
			//url:"/data.do?callcmd=APP_addAppLine",  //결재선 추가 페이지 로드
			//url:"/module/UserModule/approval/common/Modal-freeAppLine.page",
			url:"/module/usermodule/approval/common/modalFreeappline",
			type:"POST",
			data : {
				"obsSeq" : userListArray.join("_")
				,"currentGrp" : $("#hiddenApprovalGrp").val()
				,"fs" : $("#hidden_Fs").val()
			},
			dataType:"html",
			success:function(data){
				//console.log(data)
				$('#modal_freeAppLineDiv').html(data);
				$('#modal_freeAppLineDiv').localize();
				$("#modal_freeAppLinePage").modal("show");
			},
			error:function(data){
				alert("err//"+data.message);
			}
	...
	else
	{
		...
		var optIdx = $("#selectedApprovalBox option").index($("#selectedApprovalBox option:selected"));
		if(optIdx < 0 ){
			optIdx = 0;
		}
		...
		for(j = optIdx; j < optLen ; j++ ){
			...
			txtArr = $("#selectedApprovalBox option")[j].text.split(":");
			valArr = $("#selectedApprovalBox option")[j].value.split("_");
			...
			if(( type=="L" || (type=="NL" && (valArr[4] == 1 || valArr[4] == 3))) && objDataArray.length > 0) {
				...
				//결재선에 본인이 있는지 체크
				if(  objDataArray[0][5] != "0" && $("#selectedApprovalBox option[value*='_"+objDataArray[0][5]+"_']").length > 0  ){
				...
				if(chkFlag && memberFlag){
					...
					// ie에서 selectbox 오류있어 수정, 2018-02-12, 김영식
					$("#selectedApprovalBox option:eq("+j+")").val(valArr.join("_"));
					$("#selectedApprovalBox option:eq("+j+")").text(txtArr[0] + ": "+ txtArr[1]);
					$("#selectedApprovalBox option:eq("+j+")").attr("title", objDataArray[0][9])



					if (valArr[3] != noneAppUser && valArr[7]=="6")
					{
						$("#selectedApprovalBox option")[j].style.color="#00cc00";
					}
					else
					{
						$("#selectedApprovalBox option")[j].style.color="#333333";
					}
...
function fn_btnAppLineDel(){
	...
	//console.log("fn_btnAppLineDel" );

	//삭제하는 결재선의 기본값 설정
	applineSelectedLength = obj.length;

	//console.log("applineSelectedLength", applineSelectedLength );
	...
	for (i=applineSelectedLength-1; i >=0 ;i-- )
	{
			if (obj.options[i].selected==true)
			{
				...
				if (objVal[8] == "0" || objVal[8] == "1")
				{
					...
					{
						//console.log("삭제", obj.options[i].value, objVal[8])

						/*
						obj.options[i].value=objVal[0]+"_"+objVal[1]+"_"+objVal[2]+"_"+noneAppUser+"_"+objVal[4]+"_"+objVal[5]+"_"+objVal[6]+"_"+objVal[7]+"_"+objVal[8]+"_"+objVal[9];
						obj.options[i].text=objTxt[0]+": "+i18next.t("approval:setting.label.blank");
						*/

						// ie에서 selectbox 오류있어 수정, 2018-02-12, 김영식
						$(obj).find("option:eq("+i+")").val(objVal[0]+"_"+objVal[1]+"_"+objVal[2]+"_"+noneAppUser+"_"+objVal[4]+"_"+objVal[5]+"_"+objVal[6]+"_"+objVal[7]+"_"+objVal[8]+"_"+objVal[9]);
						$(obj).find("option:eq("+i+")").text(objTxt[0]+": "+i18next.t("approval:setting.label.blank"));
					}
				}
				else
				{
					showSwal(i18next.t("approval:msg.dontRemovePerson"),"warning");
				}
			}
	}
	...
	for (i=0;i<applineSelectedLength ;i++ )
	{
		...
		//console.log("재정렬", obj.options[i].value, objVal[8])
		...
		/*
		obj.options[i].value=objVal[0]+"_"+objVal[1]+"_"+objVal[2]+"_"+objVal[3]+"_"+objVal[4]+"_"+objVal[5]+"_"+objVal[6]+"_"+objVal[7]+"_"+objVal[8]+"_"+objVal[9];
		obj.options[i].text=prObjTxt[0] + " " + prObjTxt[1] +": "+objTxt[1];
		obj.options[i].title=objTxt[1];
		*/

		// ie에서 selectbox 오류있어 수정, 2018-02-12, 김영식
		$(obj).find("option:eq("+i+")").val(objVal[0]+"_"+objVal[1]+"_"+objVal[2]+"_"+objVal[3]+"_"+objVal[4]+"_"+objVal[5]+"_"+objVal[6]+"_"+objVal[7]+"_"+objVal[8]+"_"+objVal[9]);
		$(obj).find("option:eq("+i+")").text(prObjTxt[0] + " " + prObjTxt[1] +": "+objTxt[1]);
		$(obj).find("option:eq("+i+")").attr("title", objTxt[1]);
	}
...
function fn_addLineCheck(){
	if(userListArray.length > 0){
		//console.log("userListArray.length > 0");
		return true;
	}else{
		//console.log("userListArray.length <= 0");
		showSwal(i18next.t("approval:msg.noSelectUser"),"warning");
		return false;
	}
}
...
function lineSaveDML(dataArr){
	...
	$.ajax({
		...
		,data : {
			...
			,"lineSeq" : $("#lineSeq").val()
			,"dataArr" : dataArr
			,"GRPSeq"	: $("#userAppLineCurGrp").val()
		}
		,success : function(data){
			//var jData = JSON.parse(data);
			//console.log("modalAppLineUserLineDML : "+data );
			//console.log("modalAppLineUserLineDML returnType : "+data.returnType );
			//console.log("modalAppLineUserLineDML seq : "+data.seq );
			...
			if(returnType =="success" ){
				...
				if($("#lineDmlType").val() == "add" ){
					$(_id).append("<option value='"+data.seq+"'>"+$("#lineTitle").val()+"</option>");
				}else{
	...
	$.ajax({
		...
		,success : function(data){
			...
			for (i=0; i<data.data.length ; i++ ){
				...
				li = li + "<option class='   load-line' title='"+data.data[i].title+"' value='"+data.data[i].value+"' data-oldvalue='"+data.data[i].value+"'>"+data.data[i].text+"</option>"
			}
