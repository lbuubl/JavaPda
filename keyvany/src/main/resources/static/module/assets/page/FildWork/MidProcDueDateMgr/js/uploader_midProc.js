...
$(function() {
	 // Events
	 $("#uploadEvent_midProc").pluploadQueue({
        ...
        init: {
            ...
            BeforeUpload: function(up, file) {
				...
				uploader.settings.multipart_params = {
					"mc" : $("#frmMidProcEdit input[name='mc']").val(),
					"mst" : $("#frmMidProcEdit input[name='mst']").val(),
					"FK1" : $("#frmMidProcEdit input[name='FK1']").val()
				};
            },
	...
	/* 사용자 커스텀 업로드 시작버튼 */
	var uploader = new $("#uploadEvent_midProc").pluploadQueue();

	$('#btn_save_process').unbind('click');
	$('#btn_save_process').click(function(e) {
		if($("#frmMidProcEdit").valid()){

			//처리완료일이 오늘보다 큰 경우 체크
			var endDt = $("#frmMidProcEdit input[name='EndDt']").val();
			if (endDt != "" && moment(endDt).diff(moment(moment().format("YYYY-MM-DD")), "days") > 0){
				swal({
					...
					setTimeout(function(){
						$("#frmMidProcEdit input[name='EndDt']").focus();
					}, 200)
				});
				return false;
			}
	...
	document.getElementById('uploadEvent_midProc').addEventListener('drop', function(e) {
