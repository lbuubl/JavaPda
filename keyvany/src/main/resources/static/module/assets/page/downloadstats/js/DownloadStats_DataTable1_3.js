...
var t1_3_Id = "#dataList1_3";


function makeDataTable1_3(){
	...
	// ------------------------------------------------------------
		$.extend( $.fn.dataTable.defaults, {
			...
			"dom": '<"datatable-header t1_3"l><"datatable-scroll"t><"datatable-footer"ip>',
			"language": {
				//데이터 없는 경우
	...
	// Table setup
			...
			t1_3 = $(t1_3_Id).DataTable({
				...
				, "infoCallback": function( settings, start, end, max, total, pre ) {
					...
					//INDEX 역순 출력(필수)
					t1_3.column(0, {search:'applied', order:'applied'}).nodes().each( function (cell, i) {
			...
			$("div.t1_3").html("<span class='tb-header' data-i18n='DownloadStats:label.downloadHistory'></span>"+$("div.t1_3").html())

			$(t1_3_Id+' tbody').on("click" , 'tr td', function () {
				//첫번째 컬럼(Seq)이 아닌경우에만 view load
				if(!$(this).hasClass("notMove")){
						$selRow = ( t1_3.row( $(this).parent()) );
