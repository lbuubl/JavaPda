...
$(function() {
	 // Events
	 $("#uploadEvent_changed").pluploadQueue({
        ...
        init: {
            ...
            BeforeUpload: function(up, file) {
				...
				uploader.settings.multipart_params = {
					"mc" : mc,
					"MST" : $("#changedWriteForm input[name='MST']").val(),
					"FK1" : "changed"
				};
            },
            ...
            UploadComplete: function(up, files) {
				...
				//fn_afterChangedUpload();

				/*
				if($("#changedWriteForm input[name='dmlType']").val() == "add"){
					//신규 작성시 기성신청주기 변경 체크 및 confirm (기성계획 재생성 처리)
					fn_checkChangePayCheck_changed();
				}else{
					//저장처리
					fn_contractSubmitExec_changed();
				}
				*/
				fn_contractSubmitExec_changed();
	...
	/* 사용자 커스텀 업로드 시작버튼 */
	var uploader = new $("#uploadEvent_changed").pluploadQueue();

	/**********************************************************************/
	/*************************** 저장버튼 이벤트 **************************/
	/**********************************************************************/
	$('#btn_changedSave').click(function(e) {


		if($("#changedWriteForm").valid()){	//valid 체크
