...
function makeDataTable_t3(){
	var t3_Id = "#docReqReceiveList";
	...
	t3 = $(t3_Id).DataTable({
        ...
        ,"serverSide" : true
        ,"ajax": {
            //"url": "/Module/UserModule/DocMgr/json/getReqReceiveTable_json.do",
            "url": "/module/usermodule/docmgr/json/getReqReceiveTable",
			"data" : function (d) {
				d.apprStat = $("#sch_appState2").val()
			},
            "type": "POST"
        }
		...
		,"infoCallback": function( settings, start, end, max, total, pre ) {
			...
			//결재승인 대기 건수
			var waitCnt = t3.ajax.json().waitCnt;
			$("#reqReceive_waitCnt").html(waitCnt);

			//리턴값 없는경우 좌측하단 데이터 정보 안나옴
			return 'Page '+ (pageInfo.page+1) +' of '+ pageInfo.pages;
		}
		,"initComplete": function( settings, json ) {
			$("#docReqReceiveList_filter").append($('#tableFilter_appState_2').detach().show());
			$("#sch_appState2").on("change", function(){
				t3.ajax.reload(null, true);
			});

			//목록수 조정 하단이동 추가
			listSize=$("#docReqReceiveList_info");
			$("#docReqReceiveList_length").insertBefore(listSize);
	...
	// 데이터테이블 필터옵션 다국어 처리
	$(t3_Id+'_filter input[type=search]').attr('data-i18n',"[placeholder]docMgr:datatable.option.placeHolder");

	//row 클릭시 실행
	$(t3_Id+' tbody').on( 'click', 'td', function () {
		if(!$(this).hasClass("dataTables_empty")){
			var sn = t3.row($(this)).data()[1];
			fn_openDocReceive(sn);
		}
	});
