...
$(function() {
	 // Events
	 $("#uploadEvent_fieldWorkEvent").pluploadQueue({
        ...
        init: {
            ...
            BeforeUpload: function(up, file) {
				...
				uploader.settings.multipart_params = {
					"mc" : mc,
					"mst" : $("#frmWorkEventEdit input[name='mst']").val(),
					"FK1" : $("#frmWorkEventEdit input[name='FK1']").val()
				};
            },
	...
	/* 사용자 커스텀 업로드 시작버튼 */
	var uploader = new $("#uploadEvent_fieldWorkEvent").pluploadQueue();

	$('#btn_workEventSave').unbind('click');
	$('#btn_workEventSave').click(function(e) {
		...
		if($("#frmWorkEventEdit").valid()){

			var startDt		= $("#frmWorkEventEdit input[name='startDt']").val();	//발생일
			var endDueDt	= $("#frmWorkEventEdit input[name='endDueDt']").val();	//처리기한일
			var endDt		= $("#frmWorkEventEdit input[name='endDt']").val();		//처리완료일

			//발생일이 처리기한 보다 큰 경우 체크
			if (startDt != "" && endDueDt != "" && moment(startDt).diff(moment(endDueDt), "days") > 0){
				swal({
					title: "처리기한일이 발생일보다 큽니다.<br>확인하고 다시 입력해 주세요.",
			...
			//처리완료일이 발생일 보다 큰 경우 체크
			if (startDt != "" && endDt != "" && moment(startDt).diff(moment(endDt), "days") > 0){
				swal({
					title: "처리기한일이 발생일보다 큽니다.<br>확인하고 다시 입력해 주세요.",
					confirmButtonColor: "#f44336",
					closeOnConfirm: true,
	...
	document.getElementById('uploadEvent_fieldWorkEvent').addEventListener('drop', function(e) {
