...
var manTbCompany_Id = "#manCompanyList";


function makeManCompanyList(){


	//var callCmd = "/Module/SysModule/AuthMng/ManList_Company.do"
	var callCmd = "/module/sysmodule/authmng/manListCompany"
	...
	// Table setup
		...
		manTbCompany = $(manTbCompany_Id).DataTable({
			...
			, "infoCallback": function( settings, start, end, max, total, pre ) {
				//INDEX 역순 출력(필수)
				manTbCompany.column(0, {search:'applied', order:'applied'}).nodes().each( function (cell, i) {
					//console.log(total,i,start+1);
					cell.innerHTML = total-i-start+1;
				});

				//체크박스 생성
				manTbCompany.column(2, {search:'applied', order:'applied'}).nodes().each( function (cell, i) {
					//console.log(total,i,start+1);
					prop = ""
					prop = prop + " data-manSeq='"+manTbCompany.column(1).nodes()[i].innerHTML+"'"
					prop = prop + " value='"+manTbCompany.column(1).nodes()[i].innerHTML+"'"
					prop = prop + " data-userNm='"+manTbCompany.column(4).nodes()[i].innerHTML+"'"
					prop = prop + " data-part='"+manTbCompany.column(5).nodes()[i].innerHTML+"'"
				...
				$(manTbCompany_Id+'_wrapper').localize();
		...
		$(manTbCompany_Id+' tbody').on("click" , 'tr td', function () {
		...
		//자동검색 수동으로 변경
		$(manTbCompany_Id+"_filter input").unbind();
		$(manTbCompany_Id+"_filter input").keyup(function(e){
			if(e.keyCode==13){
				//console.log(this.value );
				manTbCompany.search( this.value ).draw();
				$(this).blur();
			}
		});
