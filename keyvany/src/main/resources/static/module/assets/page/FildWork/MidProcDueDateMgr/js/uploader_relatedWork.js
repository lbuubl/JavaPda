...
$(function() {
	 // Events
	 $("#uploadEvent_relatedWork").pluploadQueue({
        ...
        init: {
            ...
            BeforeUpload: function(up, file) {
				...
				uploader.settings.multipart_params = {
					"mc" : $("#frmRelatedWorkEdit input[name='mc']").val(),
					"mst" : $("#frmRelatedWorkEdit input[name='mst']").val(),
					"FK1" : $("#frmRelatedWorkEdit input[name='FK1']").val()
				};
            },
            ...
            UploadComplete: function(up, files) {
				...
				//fn_save_modal_midProcEdit();
				fn_save_modal_relatedWorkEdit();
	...
	/* 사용자 커스텀 업로드 시작버튼 */
	var uploader = new $("#uploadEvent_relatedWork").pluploadQueue();

	$('#btn_save_relatedWork').unbind('click');
	$('#btn_save_relatedWork').click(function(e) {
		if($("#frmRelatedWorkEdit").valid()){

			//처리완료일이 오늘보다 큰 경우 체크
			var endDt = $("#frmRelatedWorkEdit input[name='EndActionDt']").val();
			if (endDt != "" && moment(endDt).diff(moment(moment().format("YYYY-MM-DD")), "days") > 0){
				swal({
					...
					setTimeout(function(){
						$("#frmRelatedWorkEdit input[name='EndActionDt']").focus();
					}, 200)
				});
				return false;
			}
	...
	document.getElementById('uploadEvent_relatedWork').addEventListener('drop', function(e) {
