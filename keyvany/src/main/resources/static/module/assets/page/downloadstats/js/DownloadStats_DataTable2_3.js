...
var t2_3_Id = "#dataList2_3";


function makeDataTable2_3(){
	...
	// ------------------------------------------------------------
		$.extend( $.fn.dataTable.defaults, {
			...
			"dom": '<"datatable-header t2_3"l><"datatable-scroll"t><"datatable-footer"ip>',
			"language": {
				//데이터 없는 경우
	...
	// Table setup
			...
			t2_3 = $(t2_3_Id).DataTable({
				...
				, "infoCallback": function( settings, start, end, max, total, pre ) {
					...
					//INDEX 역순 출력(필수)
					t2_3.column(0, {search:'applied', order:'applied'}).nodes().each( function (cell, i) {
						//console.log(total,i,start+1);
						$(cell).parent().attr("title","메뉴명 : "+t2_3.row(i).data()[1] + "   작성자 : "+ t2_3.row(i).data()[3]);
			...
			$(t2_3_Id+' tbody').on("click" , 'tr td', function () {
				//첫번째 컬럼(Seq)이 아닌경우에만 view load
				if(!$(this).hasClass("notMove")){
						$selRow = ( t2_3.row( $(this).parent()) );
			...
			$("div.t2_3").html("<span class='tb-header' data-i18n='DownloadStats:label.downloadHistory'></span>"+$("div.t2_3").html())
