        ...
        init: {
            ...
            BeforeUpload: function(up, file) {
				...
				uploader.settings.multipart_params = {
					"Size" : "",
					"mc" : $("#frmTaskEdit input[name='mc']").val(),
					"mst" : $("#frmTaskEdit input[name='MST']").val(),
					"fk1" : $("#frmTaskEdit input[name='FK1']").val(),
					"fk2" : "",
					"fk3" : "",
					"Rev" : "",
					"Ak" : ""
            ...
            UploadComplete: function(up, files) {
				...
				var param = $("#frmTaskEdit").serialize();

				//T_PUBLIC_FILE.TempKey 초기화
				$.ajax({
					type :'post'										// Post로 데이터 전달(get,post방식)
					, asyn : false										// 비동기식으로 ajax 요청
					//, url: "/Module/UserModule/Work/taskDml.do"				// 전송할 페이지
					, url: "/module/usermodule/work/taskdml"				// 전송할 페이지
					, data : param
					, dataType:"json"
					, success:function( data ){
						if(data.result == "success"){

							var dmlType = $("#frmTaskEdit input[name='dmlType']").val()
							...
							loadTaskDetail(data.taskSn)	//상세페이지로 이동
							$("body").scrollTop(0);
