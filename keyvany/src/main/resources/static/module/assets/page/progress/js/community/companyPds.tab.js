define(function() {
	...
	PdsTab.prototype.load = function(classSeqno) {
		...
		var url = '/community/companyPds/comment/count/';
	...
	PdsTab.prototype.create = function(option) {
		...
		loader.get('community/companyPds.form.js?v1').then(function(form) {
		...
		loader.get('community/companyPds.grid.js').then(function(grid) {
			_this.grid = new grid({
				el: _this.gridEl,
		...
		_this.el.on('click', '.bottom_area a.btn_download', function(e) {
			...
			if (UI.getBrowser() == 'msie') {
				if(dataItems.length == 1) {
					...
					$.fileDownload('/community/companyPds/download' + params, {
