...
function makeDataTable(){
	...
	// Table setup
			...
			t1 = $(t1_Id)  .on( 'init.dt', function () {
				...
				,"serverSide" : true
				,"ajax" : {
					//"url" :"/Module/UserModule/summary/List.do"
					"url" :"/module/usermodule/summary/list"
					,"type" : "post"
					,"data" : function(d){
						d.msq = $("#msq").val()
					}
				...
				,"columnDefs": [  //컬럼 옵션 : 사용자 기호에 맞게 수정
					...
					//첫번째 컬럼은 우선순위 적용 안되는 것으로 보임..
					{
						...
						"targets": [2],
						//"width" : "120px"
						"className" : "FixColnum dt-header-center dt-body-left"
					}
				]
				//커스텀 버튼(우측 상단)
				,"buttons":[
