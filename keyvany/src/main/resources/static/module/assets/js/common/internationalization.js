...
$(function() {
		...
		.init({
			...
			load: "languageOnly",
			backend: {
				//loadPath: '/resources/module/assets/locales/{{ns}}/{{lng}}.json'
				loadPath: function(languages, namespaces){

							if(namespaces == "s"){
								return '/resources/module/assets/locales/{{ns}}/'+gg_code+'/{{ns}}_{{lng}}.json'
							}else{
								return '/resources/module/assets/locales/{{ns}}/{{ns}}_{{lng}}.json'
							}
		...
		function ChangeLanguagesDropdown(){
				...
				//업로드 다국어 적용
				loadjscssfile("/resources/module/assets/js/plugins/uploaders/plupload/i18n/"+curLan+".js","js");
		...
		$('.languageC').on('click', function () {
			...
			$.i18n.changeLanguage(choice_lan , function() {
				...
				//업로드 다국어 적용
				loadjscssfile("/resources/module/assets/js/plugins/uploaders/plupload/i18n/"+choice_lan+".js","js");
				//setUploadLan("/resources/module/assets/js/plugins/uploaders/plupload/i18n/ko.js");
		});
	...
	function fn_i18nextLoaded(){
		...
		// 로그인 페이지인 경우에만 언어셋 설정 정보 세팅.
		if(typeof loginPage !== "undefined")
		{
			if(loginPage == 'login' ){

				//var i18nActive  =    $('.language-box').find('.active').html()   ;
				var i18nActive  =    $('.language-Choice').find('.active').html()   ;

				if(i18nActive != undefined ){
					i18nActive=i18nActive.substr(10, 2);
				} else{
					i18nActive = getCookie("i18next") ; // 여기까지 안올 것임...
				}
				//alert('fn_i18nextLoaded     : ' + loginPage + '  /     : '+i18nActive ) ;

				$('.'+i18nActive).parent().addClass('active');

				$('.language-box').children('.dropdown-toggle').html(
					' <i class="caret" />' + $('.language-box').find('.'+i18nActive).html()
				)
			}
