...
tinymce.PluginManager.add('IE_Print', function(editor) {
	var settings = editor.settings, sandbox = !tinymce.Env.ie;

	editor.addCommand('mceIE_Print', function() {
		editor.windowManager.open({
			...
			onPostRender: function() {
				...
				var printScript = (
					...
					'	 }catch (e) {'+
					'		  alert("- 도구 > 인터넷 옵션 > 보안 탭 > 신뢰할 수 있는 사이트 선택   1. 사이트 버튼 클릭 > 사이트 추가   2. 사용자 지정 수준 클릭 > 스크립팅하기 안전하지 않은 것으로 표시된 ActiveX 컨트롤 (사용)으로 체크 ※ 위 설정은 프린트 기능을 사용하기 위함임") ;'+
					'	  } '+
					'}</script> '
				);
				...
				previewHtml = (
					...
					'<html>' +
					'<head>' +
					' <script type=text/javascript src=/module/tinymce/js/tinymce/tinymce.js></script> '+	headHtml +
					'</head>' +
					'<body id="' + bodyId + '" class="mce-content-body ' + bodyClass + '"' + dirAttr + '>' +
						editor.getContent({format : 'raw'}) +
						preventClicksOnLinksScript + printScript +
					'</body>' +
					'</html>'
				);
	...
	editor.addButton('IE_Print', {
		title: 'IE_Print',
		cmd: 'mceIE_Print'
	});

	editor.addMenuItem('IE_Print', {
		text: 'IE_Print',
		cmd: 'mceIE_Print',
		context: 'view',
		icon: 'print'
